...
<div class="space-y-5 lg:space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <a href="{{ route('zones.show', $zone->id) }}" class="text-sm font-medium hover:underline mb-1 inline-block" style="color: var(--primary);">← Back to zone</a>
            <h1 class="font-display font-semibold text-2xl lg:text-3xl" style="color: var(--ink);">Edit zone</h1>
            <p class="text-sm mt-1" style="color: var(--ink-muted);">Update zone details.</p>
        </div>
    </div>

    <div class="rounded-xl border p-5 lg:p-6 max-w-xl" style="background: var(--bg-surface); border-color: var(--border);">
        <form action="{{ route('zones.update', $zone->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="zone_number" name="zone_number" value="{{ old('zone_number', $zone->zone_number) }}" required class="w-full rounded-lg border py-2 px-3 text-sm focus:outline-none focus:ring-2 transition" style="border-color: var(--border); color: var(--ink); --tw-ring-color: var(--primary);">
